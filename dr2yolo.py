...
"coffee","coke","milk",'edible oil','oyster','soy_sauce'
    ...
    def oneDr2Yolo(self,txt):
        with open(os.path.join(self.drpath,txt),"r",) as f:
            ...
            for idx,m in enumerate(message):
                cls,x,y,w,h,conf = int(m[0]),float(m[1]),float(m[2]),\
                ...
                result.append("{} {} {} {} {} {}".format(claname,conf,x0,y0,x1,y1));
        f.close()
...
if __name__ == "__main__":
    import sys
    if len(sys.argv)<3:
        print("python dr2yolo.py [drpath] [outpath]")
        exit()
    drpath = sys.argv[1]
    outpath = sys.argv[2]

    dr2yolo = DR2YOLO(drpath=drpath,outpath=outpath,orgh=ORGH,orgw=ORGW);
    dr2yolo.dirDr2yolo();
